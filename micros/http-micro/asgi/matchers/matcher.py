...
class Matcher:
    ...
    @staticmethod
    def match(path_patterns: List[str], actual_path: str) -> Tuple[bool, str, Dict]:
        for path_pattern in path_patterns:
            if path_pattern == Matcher.RESOURCE_PATH:
            ...
            path_parameters = {key.strip("{}"): val for key, val in different_paths.items() if key.startswith("{") and key.endswith("}")}
